...
class UserTopicModelingSystem:
    ...
    def run_full_pipeline(self,
        ...
        try:
            ...
            if self.calculate_temporal_topic_data() is None: 
                print("Warning: Could not calculate temporal data.")
            else:
                # --- ADD THIS SECTION TO SAVE TEMPORAL DATA ---
                if self.temporal_topic_data:
                    temporal_pickle_path = output_path / "temporal_topic_data.pkl"
                    try:
                        with open(temporal_pickle_path, 'wb') as f_pkl:
                            pickle.dump(self.temporal_topic_data, f_pkl)
                        print(f"   Temporal topic data saved to {temporal_pickle_path}")
                        results['temporal_data_saved'] = str(temporal_pickle_path)
                    except Exception as e_pickle:
                        print(f"   Error saving temporal topic data: {e_pickle}")
                else:
                    print("   No temporal topic data to save.")
        ...
        final_results = {
            ...
            'temporal_data_saved_path': results.get('temporal_data_saved', None)
        }
        if detect_suspicious:
