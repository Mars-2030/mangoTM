...
class UserTopicModelingSystem:
    ...
    def calculate_temporal_topic_data(self) -> Dict[str, pd.DataFrame]:
        ...
        for user_id in self.doc_meta['user_id'].unique():
            ...
            for time_bin in user_docs[time_col].unique():
                ...
                try:
                    bin_topic_dist = user_topic_dists[bin_indices.tolist()]
                    user_temporal[time_bin] = bin_topic_dist.mean(axis=0)
                except Exception as e:
                    # Handle index calculation errors
                    try:
                        rel_indices = [list(user_docs.index).index(i) for i in bin_indices]
                        bin_topic_dist = user_topic_dists[rel_indices]
                        user_temporal[time_bin] = bin_topic_dist.mean(axis=0)
                    except Exception as e2:
                        print(f"Warning: Could not calculate temporal data for user {user_id}, time {time_bin}: {e2}")
                        continue
    ...
    def visualize_topic_embedding(self, method: str = 'tsne', save_path: Optional[str] = None):
        ...
        if not has_dim_reduction:
            print("UMAP or scikit-learn not installed. Skipping visualization.")
            return
    ...
    def export_topic_words(self, output_path: str, n_words: int = 20):
        ...
        for topic_id in range(self.num_topics):
            words = self.lda_model.show_topic(topic_id, n_words)
        ...
        print(f"Exported topic words to {output_path}")
    ...
    def run_full_pipeline(self, 
        ...
        # Export topic words
        self.export_topic_words(f"{output_dir}/topic_words.csv")
        ...
        if visualize_topics:
            ...
            for user_id in top_users:
                try:
                    ...
                    print(f"Warning: Could not create visualization for user {user_id}: {e}")
